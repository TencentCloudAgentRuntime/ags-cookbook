...
- `deployment-cookbook` — Deployment management with `agr`, from httpbin basics to a persistent agent workspace
...
- `multi-user-secret` — per-user ManagedSecret distribution via AGS CredentialProvider (unhosted Agent)
...
| `deployment-cookbook` | advanced | agr CLI + Markdown | Follow a scenario README | Covers deployment, scaling, lifecycle, affinity, and a persistent agent workspace |
...
| `multi-user-secret` | advanced | Python + AGS CredentialProvider | `make run` | Per-user ManagedSecret distribution for unhosted Agent scenarios |
