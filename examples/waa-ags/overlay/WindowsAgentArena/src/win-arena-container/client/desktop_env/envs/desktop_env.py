"""
Derived from microsoft/WindowsAgentArena under MIT.
Modified and redistributed by Agent Sandbox Cookbook as part of the WAA AGS overlay.
"""
...
class DesktopEnv(gym.Env):
    ...
    def __init__(
        ...
        self.ags_remote = os.environ.get("WAA_AGS_REMOTE", "").lower() in ("1", "true", "yes")
        if self.ags_remote:
            self.vm_controller = None
            logger.info("Using AGS remote Windows sandbox; QMP controller is disabled.")
        else:
    ...
    def _get_screenshot(self):
        if self.ags_remote:
            ...
            if screenshot is None:
                logger.error("Failed to get screenshot from AGS Windows server!")
            return screenshot
    ...
    def close(self):
        ...
        if self.ags_remote:
            logger.info("AGS sandbox lifecycle is managed by run_ags.py")
        elif self.remote_vm:
            # TODO: Implement this
