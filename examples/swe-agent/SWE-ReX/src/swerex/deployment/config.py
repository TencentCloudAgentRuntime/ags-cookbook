...
class TencentAGSDeploymentConfig(BaseModel):
    """Configuration for Tencent Cloud AGS (Agent Sandbox) deployment."""
    ...
    domain: str = Field(default="ap-chongqing.tencentags.com", description="Domain for sandbox endpoint")
    ...
    # Role configuration
    role_arn: str = Field(default="", description="Role ARN for accessing container registry (e.g., 'qcs::cam::uin/3321337994:roleName/tcr-readonly-ags-test')")

    # Storage mount configuration (optional)
    # For StorageMounts in CreateSandboxTool (define the mount source)
    mount_name: str = Field(default="", description="Name of the mount (used in both StorageMounts and MountOptions)")
    mount_image: str = Field(default="", description="Image to mount as storage (e.g., swerex-tools image)")
    mount_image_registry_type: str = Field(default="enterprise", description="Registry type for mount image (enterprise, personal, etc.)")
    # For MountOptions in StartSandboxInstance (use the mount)
    mount_path: str = Field(default="/nix", description="Path to mount the storage")
    image_subpath: str = Field(default="/nix", description="SubPath within the image mount (optional)")
