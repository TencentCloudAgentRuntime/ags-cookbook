...
4. 增加显式的 `--allow-remote-management` 开关。只有同时开启该开关并通过 `trustedHosts` 校验的请求，才能访问上游默认限制在 loopback 的设置、凭据、模型发现、Preset 管理和其他管理 RPC；未开启时保留上游默认限制。
