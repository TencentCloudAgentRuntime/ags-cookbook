# 将 Brain Agent 多轮会话接入 Agent Runtime 云端 Session 服务
...
如需了解 Hands Deployment 如何通过 Agent Runtime 云端 Session 服务延续访问原 workspace 并隔离不同会话，请参见 [Hands Deployment 会话接入教程](../hands/README_zh.md)。

随教程提供的 [`SessionPersistence` 插件](./plugin/index.js)负责创建和恢复 Session、写入 Events，并支持 DSH 从 Session 恢复上下文。除了直接使用示例镜像，使用自有 DSH 镜像时也可以集成该插件。
...
直接验证本教程时，使用 `DSH_SESSION_IMAGE` 指定的公开示例镜像即可。该镜像已集成本教程的 [`SessionPersistence` 插件](./plugin/index.js)，会将 DSH 多轮会话和执行事件持续保存到 Agent Runtime 云端 Session 服务。
