# Connect Brain agent conversations to the Agent Runtime cloud Session service
...
To continue using an existing Hands Deployment workspace and isolate workspaces between Sessions through the Agent Runtime cloud Session service, see the [Hands Deployment session integration tutorial](../hands/README.md).

The included [`SessionPersistence` plugin](./plugin/index.js) creates and restores Sessions, writes Events, and lets DSH restore context from Session. You can use the example image directly or integrate the plugin into your own DSH image.
...
export DSH_SESSION_IMAGE='ccr.ccs.tencentyun.com/ags.dev/deepseek-harness:v0.1.1-rc.2-ags.4-session.1'
...
To run this tutorial, use the public example image specified by `DSH_SESSION_IMAGE`. It includes the tutorial's [`SessionPersistence` plugin](./plugin/index.js), which continuously persists DSH multi-turn conversations and execution events to the Agent Runtime cloud Session service.
