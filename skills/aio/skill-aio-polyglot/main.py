...
from ags_client import BrowserSandboxSession, build_port_url, credential_mode, create_tool, delete_tool
...
_UVICORN_PORT = 49999
# default AIO tool — override via AGS_AIO_TOOL_NAME env var if your account
# uses a different tool name for the AIO sandbox image.
_DEFAULT_TOOL = os.environ.get("AGS_AIO_TOOL_NAME", "aio-v1")
...
if __name__ == "__main__":
    # If AGS_AIO_TOOL_NAME is set, use it directly; otherwise create a
    # temporary aio tool via the AKSK control plane, run the demo, then delete it.
    tool_name = _DEFAULT_TOOL
    created_tool_id: str | None = None

    if tool_name == "aio-v1" and not os.environ.get("AGS_AIO_TOOL_NAME"):
        if credential_mode() != "aksk":
            raise SystemExit(
                "No AGS_AIO_TOOL_NAME set and no AKSK credentials available. "
                "Either set AGS_AIO_TOOL_NAME or provide TENCENTCLOUD_SECRET_ID/"
                "TENCENTCLOUD_SECRET_KEY to auto-create a temporary aio tool."
            )
        log.info("Creating temporary aio tool via AKSK control plane...")
        import time
        import uuid
        tmp_name = f"agsskill-aio-{uuid.uuid4().hex[:8]}"
        created_tool_id = create_tool(tool_name=tmp_name, tool_type="aio")
        tool_name = tmp_name
        log.info("Temporary aio tool created: %s (id=%s), waiting for ACTIVE...",
                 tool_name, created_tool_id)
        # Wait up to 30s for tool to become ACTIVE
        for _ in range(15):
            time.sleep(2)
            from ags_client import make_ags_client
            from tencentcloud.ags.v20250920 import models as _m
            _req = _m.DescribeSandboxToolListRequest()
            _resp = make_ags_client().DescribeSandboxToolList(_req)
            for t in (_resp.SandboxToolSet or []):
                if t.ToolId == created_tool_id and t.Status == "ACTIVE":
                    log.info("Tool %s is ACTIVE.", created_tool_id)
                    break
            else:
                continue
            break
        else:
            log.warning("Tool did not reach ACTIVE in 30s, proceeding anyway.")

    try:
        log.info("=== Demo: run_code across 5 languages (tool=%s) ===", tool_name)
        demos = [
        ...
        for lang, code in demos:
            r = run_code(code, language=lang, tool_name=tool_name)
            print(f"\n=== Skill Result: run_code ({lang}) ===")
            print(json.dumps(r, indent=2, ensure_ascii=False))
    finally:
        if created_tool_id:
            log.info("Deleting temporary aio tool: %s", created_tool_id)
            delete_tool(created_tool_id)
            log.info("Temporary aio tool deleted.")
