...
def pause_instance(instance_id: str, mode: str | None = None) -> None:
    ...
    if mode:
        if hasattr(ags_models.PauseSandboxInstanceRequest, "Mode"):
            req.Mode = mode
        else:
            log.warning("pause_instance: Mode=%s requested but SDK model "
                        "does not support it yet; using default.", mode)
...
def start_instance(
    ...
    # NOTE: AutoPause / AutoResume are documented in the AGS API but not yet
    # supported by the installed tencentcloud-sdk-python-ags model.  Setting
    # them is a no-op until the SDK is updated — we log a warning and skip.
    if auto_pause:
        if hasattr(ags_models.StartSandboxInstanceRequest, "AutoPause"):
            req.AutoPause = True
        else:
            log.warning("start_instance: AutoPause requested but SDK model "
                        "does not support it yet; ignoring.")
    if auto_resume:
        if hasattr(ags_models.StartSandboxInstanceRequest, "AutoResume"):
            req.AutoResume = True
        else:
            log.warning("start_instance: AutoResume requested but SDK model "
                        "does not support it yet; ignoring.")
