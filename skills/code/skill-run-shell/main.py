...
def _run_one(sbx, cmd: str, cwd: str | None, envs: dict | None, timeout: int) -> dict:
    ...
    #
    # The SDK raises ``CommandExitException`` for non-zero exit codes.  We catch
    # it to return the exit code + output as a *normal* result — the calling Agent
    # decides how to handle non-zero exits.
    try:
        ...
        # CommandExitException carries .exit_code, .stdout, .stderr on the exc
        # itself; other exceptions become exit_code=-1 with the error in stderr.
