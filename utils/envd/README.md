...
envd first inherits envd's complete process environment. Explicit envd
defaults, `/init` variables, and per-request variables are appended afterward,
so the more specific value wins.
...
credentials or other values that sandbox commands must not read. Use `/init`
or per-request environment variables when only a small allowlist should be
propagated.
