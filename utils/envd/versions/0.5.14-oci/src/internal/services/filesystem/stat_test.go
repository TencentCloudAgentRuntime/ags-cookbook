...
func TestStatUsesStartupIdentityWithoutAuthorization(t *testing.T) {
	t.Parallel()

	u, err := user.Current()
	require.NoError(t, err)

	path := filepath.Join(t.TempDir(), "startup-user.txt")
	err = os.WriteFile(path, []byte("startup identity"), 0o644)
	require.NoError(t, err)
