# envd 0.5.14-oci source provenance
...
listed below and packages the result as `0.5.14-oci`.

It is a sibling of `0.5.14-modified`, which it does not modify: that
distribution keeps its original behavior for existing users, and the OCI
User/Workdir defaults described here live only in this directory.
...
valid version: the directory name `0.5.14-oci` is **not** parseable by
...
```bash
make -C utils/envd test VERSION=0.5.14-oci
make -C utils/envd test-privileged VERSION=0.5.14-oci
make -C utils/envd test-race VERSION=0.5.14-oci
make -C utils/envd build VERSION=0.5.14-oci
```
...
actually run. `make -C utils/envd test-privileged VERSION=0.5.14-oci` runs a
